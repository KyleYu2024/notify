...
// Debug 输出调试日志
func (l *PluginLogger) Debug(format string, args ...any) {
	msg := fmt.Sprintf("[Plugin:%s] "+format, append([]any{l.pluginID}, args...)...)
	logger.Debug(msg)
}
...
// Error 输出错误日志
func (l *PluginLogger) Error(format string, args ...any) {
	msg := fmt.Sprintf("[Plugin:%s] "+format, append([]any{l.pluginID}, args...)...)
	logger.Error(msg)
}
...
func (m *Manager) loadOne(pluginDir string) error {
	...
	// 为插件设置专用日志器
	pluginLogger := NewPluginLogger(config.ID)
	instance.SetLogger(pluginLogger)
